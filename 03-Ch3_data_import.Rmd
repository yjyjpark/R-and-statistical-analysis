...
1) HTML 테이블을 R로 불러와 다음과 같이 출력해 보자. 
...
2) 변수 이름을 다음과 같이 `rank`, `item`, `value`로 변경하자. 
...
3) 변수 `value`는 각 품목의 수출액이다. 각 품목의 수출액이 20개 전체 품목의 총 수출액에서 차지하는 비율을 나타내는 변수 `prop`를 다음과 같이 생성해 보자. 
...
4) 변수 `prop`의 값이 5 이상인 품목만을 선택해서 다음의 형태로 출력해 보자. 
...
5) 변수 `prop`의 값이 5 이상인 품목의 평균 수출액을 계산해 보자. 
...
**4.** 파일 `airs.csv`에는 1973년 5월 뉴욕시에서 측정된 공기의 질과 관련된 자료가 입력되어 있다. 

1) R로 불러와서 다음과 같이 출력해 보자. 

```{r, echo=FALSE}
airs <- read_csv("Data/airs.csv")
```
...
2) 네 변수들의 평균을 다음의 형태로 출력해 보자. 평균값이 `NA`로 나온 변수가 있는 이유는 무엇인가?

```{r, echo=FALSE}
colMeans(airs[-1])
```

3) 변수의 평균값이 `NA`가 되지 않도록 조치해서 다음의 형태로 다시 출력해 보자.  

```{r, echo=FALSE}
colMeans(airs[-1], na.rm = TRUE)
```

4) 변수 `Ozone`과 `Solor.R`의 결측값이 있는 행은 모두 제거하고, 그 결과를 `airs_1`에 할당하라. 

```{r, echo=FALSE}
airs_1 <- airs[!(is.na(airs$Ozone)|is.na(airs$Solar.R)),]
```

5) `airs_1`의 변수 `Ozone`이 평균값 이상인 그룹과 평균값 미만인 그룹으로 구분해 주는 변수 `Oz.Gp`를 생성해서, 다음과 같이 `airs_1`에 추가하자.  
...
6) `airs_1`의 변수 `Oz.Gp`의 값이 `High`인 그룹과 `Low`인 그룹에서 변수 `Solor.R`, `Wind`, `Temp`의 평균값에 어떤 차이가 있는지 확인하고, 그 효과를 설명해 보자. 
...
**5.** 파일 `cars.csv`에는 234대 자동차의 제조회사, 모델, 엔진 기통 수, 변속기 종류, 연비가 입력되어 있다. 
...
2) 변수 `manufacturer`와 `model`을 변수 `maker`로 통합해 보자. 두 변수 값을 통합할 때 구분자는 `"-"`을 사용하라. 통합 결과는 다음과 같다. 
...
3) 변수 `maker`의 값이 동일한 자동차가 많이 입력되어 있다. 변수 `maker`에는 몇 종류의 서로 다른 자동차 모델이 입력되어 있는지 확인해 보자. 

4) 변수 `cyl`의 값이 4이고 `trans`의 값이 `"auto"`인 자동차의 대수와 변수 `cyl`의 값이 8이고 `trans`의 값이 `"auto"`인 자동차의 대수를 확인하자. 
...
5) 변수 `cyl`의 값이 4이고 `trans`의 값이 `"auto"`인 자동차의 변수 `mpg`의 평균값과 변수 `cyl`의 값이 8이고 `trans`의 값이 `"auto"`인 자동차의 변수 `mpg`의 평균값을 계산하고 비교해 보자. 
