...
Figure \@ref(fig:barely)은 자료 시각화의 위력을 잘 보여주는 예로서 @c1993 에 의한 놀라운 발견이다. 
...
Figure \@ref(fig:first-plot)에서는 점의 위치만 매핑이 되었다.
...
이런 경우에는 다른 시각적 요소를 `drv`에 함께 매핑하여 작성한 Figure \@ref(fig:5-4-b)가 좋은 대안이 될 수 있다.
...
Figure \@ref(fig:5-17)은 너무 많은 점들이 서로 겹쳐 있어서 관계를 확인하는 데 약간의 어려움이 있으며, 따라서 좋지 않은 그래프라고 하겠다.
...
Figure \@ref(fig:5-18-1)과 Figure \@ref(fig:5-18-2)는 데이터 프레임 `mpg`의 변수 `displ`과 `hwy`를 대상으로 `geom_point()`와 `geom_smooth()`를 각각 적용한 결과를 보여주고 있다.
...
Figure \@ref(fig:5-geom-text-1)의 산점도에 변수 `cty`의 값이 큰 두 점 주변에 라벨을 추가해 보자. 
...
Figure \@ref(fig:5-geom-text-4)의 경우에는 `hjust = "left"`와 `vjust = "top"`을 지정한 것인데, 
...
변수 `hwy`의 값이 40이 넘는 자료가 Figure \@ref(fig:5-geom-text-5)에는 2개로 표시되었지만, 사실은 3개의 자료가 있는 것을 알 수 있다. 
...
`position="jitter"`를 추가하면 되는데, 그 결과는 Figure \@ref(fig:5-26-2)와 같다.
...
Figure \@ref(fig:5-31-2)는 다음과 같이 조건 변수 `class`를 `y` 변수로 사용해서 작성할 수 있다.  
...
또한 Figure \@ref(fig:5-31-1)는 `class`를 `y` 변수로 지정하고 함수 `coord_flip()`을 다음과 같이 사용해서 작성할 수 있다. 
